...
class excerciseType(BodyPartAngle):
    ...
    def calculate_exercise(self, exercise_type, counter, status):
        if exercise_type == "push-up":
            counter, status = excerciseType(self.landmarks).push_up(
                counter, status)
        elif exercise_type == "pull-up":
            counter, status = excerciseType(self.landmarks).pull_up(
                counter, status)
        elif exercise_type == "squat":
            counter, status = excerciseType(self.landmarks).squat(
                counter, status)
        elif exercise_type == "walk":
            counter, status = excerciseType(self.landmarks).walk(
                counter, status)
        elif exercise_type == "sit-up":
            counter, status = excerciseType(self.landmarks).sit_up(
                counter, status)
